...
public static class MigratorCommand
{
    ...
    private sealed class CliUsageException(string option) : Exception { public string Option { get; } = option; }
    private static int Execute(string[] args, TextWriter output)
    {
        ...
        var command = args[0];
        if (!new[] { "list", "status", "validate", "migrate", "rollback", "plan", "sql" }.Contains(command)) throw new CliUsageException("command");
        ...
        for (var i = 1; i < args.Length; i++)
        {
            ...
            else throw new CliUsageException(key);
        }
        string Value(string key, string fallback = null) => values.GetValueOrDefault(key, fallback);
        T EnumValue<T>(string key, string fallback) where T : struct, Enum => Enum.TryParse<T>(Value(key, fallback), true, out var result) && Enum.IsDefined(result) ? result : throw new CliUsageException(key);
        var providerType = EnumValue<ProviderTypes>("--provider", "none");
        if (providerType == ProviderTypes.none) throw new CliUsageException("--provider");
        var assemblyPath = Path.GetFullPath(Value("--assembly") ?? throw new CliUsageException("--assembly"));
        ...
        try
        {
            ...
            var target = Value("--target") is { } targetString ? long.TryParse(targetString, out var parsed) && parsed >= 0 ? parsed : throw new CliUsageException("--target") : versioned.Select(MigrationLoader.GetMigrationVersion).DefaultIfEmpty(0).Max();
            if (command == "rollback" && !values.ContainsKey("--target")) throw new CliUsageException("--target");
            if (command == "list")
            {
            ...
            {
                if (command != "sql" || values.ContainsKey("--profiles") || types.Any(t => t.GetCustomAttribute<MaintenanceAttribute>() != null)) throw new UnsupportedMigrationFeatureException("CLI operation is unsupported.");
            ...
            var connectionString = Environment.GetEnvironmentVariable(Value("--connection-env", "MIGRATOR_CONNECTION")) ?? throw new CliUsageException("--connection-env");
            ...
            return 0;
            int Seconds(string key, string fallback) => int.TryParse(Value(key, fallback), out var seconds) && seconds >= 0 ? seconds : throw new CliUsageException(key);
