...
| Custom C# logic                          | `Up` / `Down` or fluent `BuildUp` / `BuildDown`; provider context                                     | Migration code / connection operations     | SQL/custom operations for database work               | `IScript` and command factory         | Surrounding host logic; migrations are SQL |
